...
packages <- c("pryr", "tictoc",  # profiling
              ...
              "dendextend", "plyr", "knitr", "kableExtra", "stargazer",
...
df6 %>% 
  ...
  count(building_type) %>% 
  arrange(desc(n)) %>% 
  View("Observations per building class")
  ...
  df6 %>% 
    ...
    count(building_type) %>% 
    arrange(desc(n)) %>% 
    kable("latex", caption = "List of detailed buildings with their corresponding number of observations", booktabs = T) %>%
...
summary(df8$price_m2)
  hist(df8$price_m2, breaks = 10000,  main = "Histogram", xlim = c(0, 40000),
       xlab = "Price per sqm (cut off at $40'000)")
dev.off()
...
df8 %>% 
  ...
  View("Mean price per building class")

# Check if the most expensive properties are reasonable
df8 %>% 
  top_n(25, price_m2) %>% 
  arrange(desc(price_m2)) %>%
  View("Most expensive observations")  # data seems reasonable for the location, but are subject to a submarket

  # Generate latex output
  df8 %>% 
    top_n(25, price_m2) %>% 
    arrange(desc(price_m2)) %>%
    dplyr::select(address, borough, price, m2, age, building_type, price_m2) %>% 
    kable("latex", caption = "List of the most expensive properties transacted in 2018", booktabs = T) %>%
    kable_styling(latex_options = c("striped", "HOLD_position"))

# Check anomalies
df8 %>% filter(address == "28 CLIFTON PLACE" |
               address == "22 CLIFTON PLACE") %>%
  View("Anomalies")
...
# tic("k-means: ")
#   km <- kmeans(df8$price_m2, 2)
#   df.km <- cbind(df8$price_m2, km$cluster) %>%
#     set_colnames(c("price_m2", "cluster")) %>%
#     as.data.frame() %>%
#     arrange(price_m2)
# toc()
...
#   plot(df8$price_m2, pch = 19, col = km$cluster, ylab = "Price per sqm", main = "K-means clustering")
...
#   plot(sort(scale(df8$price_m2)), ylab = "z-value", main = "z-Score outlier detection", pch = 16)
#   mu <- mean(df8$price_m2) ; sigma <- sd(df8$price_m2)
...
#   boxplot(df8$price_m2, main = "Box plot (price per sqm)")  # extreme outliers upwards
#   abline(h = cut.kmeans, col = "red")
#   hist(df8$price_m2,
#        breaks = seq(min(df8$price_m2), max(df8$price_m2), length.out = 1000),
#        xlab = "Price per sqm",
#        main = "Histogram (price per sqm)",
#        col = "black")
#   abline(v = cut.kmeans, col = "red")
#   barplot(sort(df8$price_m2), main = "Barplot (Price per sqm, sorted)", ylab = "USD", xlab = "Price per sqm")
#   abline(h = cut.kmeans, col = "red")
# dev.off()
# 
...
tic("Mapping")
  tmap_mode("plot")
  pdf("/Users/samu_hugo/Desktop/Code/Plots/Statistics/map-prices.pdf")
  tm_shape(sf.boroughs) + 
    tm_polygons(alpha = 0.1, border.alpha = 0.4) +
    tm_layout(title = "2018 Property prices", title.size = 2, title.position = c(0.1, 0.95),
              frame = F) + 
    tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
    tm_shape(sf.nyc) +
    tm_dots(size = 0.065, col = "price_m2", title = "Price per m2", alpha = 1, 
...
sf.nyc_1200 <- st_buffer(sf.nyc, 1200)
sf.nyc_1600 <- st_buffer(sf.nyc, 1600)
...
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-high-schools.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "High schools", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
...
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-elementary-schools.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) + 
  tm_layout(title = "Elementary schools", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Count number of shootings within 20min walking distance
...
# Plot shootings
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-shootings.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Shootings", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-subway.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Subway stations", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Count number of noise complaints within 20min walking distance
sf.nyc$n_noise <- st_contains(sf.nyc_800, sf.noise) %>% 
  map(length) %>% 
  unlist()

# Plot noise complaints
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-noise.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Noise complaints", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-boroughs.pdf")
tm_shape(sf.boroughs) +
...
# Plot restaurants
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-restaurants.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Restaurants", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Plot cafes
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-cafes.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Cafes", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Plot bars
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-bars.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Bars", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Plot cinemas
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-cinemas.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Cinemas", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Plot airports
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-airports.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Airports", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-fitness.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Fitness centres", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
#   tm_shape(sf.boroughs) + 
#   tm_polygons(alpha = 0.05, border.alpha = 0.4) +
#   tm_shape(sf.nyc) +
#   tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
#           style = "quantile", border.lwd = NA, border.col = NA,
#           palette = viridis::magma(n = 20, 
...
#   tm_polygons(border.col = "green", alpha = 0.2)
...
# Plot park areas
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-parks.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Park areas", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
#                                    end = 0.9, 
#                                    direction = -1)) +
#   tm_shape(sf.playground$osm_points) +
#   tm_dots(size = 0.05, col = "blue", alpha = 1) +
#   tm_shape(sf.playground$osm_polygons) +
#   tm_dots(size = 0.05, col = "green", alpha = 1) +
#   tm_shape(sf.playground$osm_polygons) +
#   tm_polygons(border.col = "green", alpha = 0)
...
# Plot playgrounds
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-playgrounds.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Playgrounds", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Plot railways
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-railways.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Railways", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-shopping.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Shopping centres", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Plot grocery stores
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-grocery-stores.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Grocery stores", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Plot stock exchange
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-exchange.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Stock Exchange", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Plot roads
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-roads.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Road network", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
tmap_mode("plot")
pdf("/Users/samu_hugo/Desktop/Code/Plots/Variables/map-waterfront.pdf")
tm_shape(sf.boroughs) + 
  tm_polygons(alpha = 0.1, border.alpha = 0.4) +
  tm_layout(title = "Waterfront access", title.size = 2, title.position = c(0.1, 0.95),
            frame = F) + 
  tm_legend(title.size = 1.6, text.size = 1, position = c(0.1, 0.65)) + 
  tm_shape(sf.nyc) +
  tm_dots(size = 0.1, col = "price_m2", title = "Price per square metre", alpha = 0.4, 
...
# Saving data set
save(sf.nyc, file = "/Users/samu_hugo/Desktop/Code/data-variables-full.RData")
